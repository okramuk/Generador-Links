...
function cambiarClave2()
{
	old_passwd = document.getElementById("old_passwd").value;
	new_passwd = document.getElementById("new_passwd").value;
	new_passwd_confirm = document.getElementById("new_passwd_confirm").value;
	pwd = document.getElementById("pwd").value;
	type_user = "";
	//alert (document.getElementById("old_passwd").value);
	//alert (document.getElementById("new_passwd").value);
	//alert (	document.getElementById("new_passwd_confirm").value);

	//alert('cambiarClave2 ='+old_passwd+','+new_passwd+','+new_passwd_confirm+','+pwd) ;
	if(validar(old_passwd,new_passwd,new_passwd_confirm))
	{
		// Creo el nuevo objeto AJAX y envio al servidor el ID del select a cargar y la opcion seleccionada del select origen
		var ajax=nuevoAjax();
		ajax.open("GET", "./clases/cambiarClave.php?old_passwd="+old_passwd+"&new_passwd="+new_passwd+"&new_passwd_confirm="+new_passwd_confirm+"&type_user="+type_user+"&pwd="+pwd+"&accion=cambiarClave",true);
